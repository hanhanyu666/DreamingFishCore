...
- `BUILDABLE`：玩家保持生存模式，可以正常建造、破坏、使用容器、书架和活塞。只拦截 TNT、岩浆放置/流入，以及打火石点燃下界传送门；普通火焰、苦力怕等其他正常世界行为和水可以使用，剧情 NPC 与装饰仍受保护。
...
冒险模式是 `PROTECTED` 地点的第一层低成本保护，方块事件仍作为模组方块路径的服务端兜底；容器、书架和普通交互不再被地点事件层拦截。两种模式都只额外拦截 TNT、岩浆，以及会实际生成下界传送门的打火石操作。

普通玩家在 `PROTECTED` 地点内不能：

- 破坏或放置方块；
- 放置或引爆 TNT、放置或流入岩浆、用打火石点燃下界传送门；
- 伤害剧情 NPC、盔甲架和悬挂装饰实体。

在 `BUILDABLE` 地点内，普通的方块建造、破坏、容器/书架使用和活塞操作允许进行；水、普通火焰以及苦力怕等非 TNT 爆炸也按原版规则处理。不能放置或引爆 TNT，不能放置岩浆，也不能用打火石点燃会生成的下界传送门；已有岩浆也不能流入或流出可建造地点。故事区域外不受这些故事地点建筑规则影响，仍由服务器和 EconomySystem 的普通规则处理。

保护不会禁止玩家与敌对怪物战斗；两种地点都只过滤 TNT、岩浆和用打火石点燃下界传送门，其他爆炸、火焰和流体按各自游戏模式的正常规则处理。创造模式只绕过普通方块保护，TNT、岩浆和打火石传送门点火禁令仍然对所有游戏模式生效；OP 权限仅用于管理命令，OP 以生存模式参与时与普通玩家遵守相同规则。剧情 NPC 的所有伤害仍会被拦截，应通过未来的剧情状态接口处理剧情杀。
