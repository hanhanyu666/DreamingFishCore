package com.hhy.dreamingfishcore.gameplay.playerattributes_system.death.client.cache;
...
public class DeathScreenDataStorage {
    ...
    public static void setData(float respawnPoint, float normalCost, float keepInventoryCost, boolean isInfected, Component deathMessage,
                             double deathX, double deathY, double deathZ, String dimension, UUID deathId) {
        storedData = new DeathScreenData(respawnPoint, normalCost, keepInventoryCost, isInfected,
    ...
    public static DeathScreenData getData() {
        if (storedData == null) {
            // 返回默认数据，确保始终显示自定义界面
            return new DeathScreenData(0.0f, 5.0f, 35.0f, false,
                    Component.literal("正在等待服务器同步死亡数据"),
                    0, 64, 0, "minecraft:overworld", new UUID(0L, 0L));
        }
        return storedData;
    }
    ...
    public record DeathScreenData(
        ...
        public DeathScreenData(float respawnPoint, float normalCost, float keepInventoryCost, boolean isInfected, Component deathMessage) {
            this(respawnPoint, normalCost, keepInventoryCost, isInfected, deathMessage,
                    0, 64, 0, "minecraft:overworld", new UUID(0L, 0L));
        }
    }
}
