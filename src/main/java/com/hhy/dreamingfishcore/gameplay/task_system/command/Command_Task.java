package com.hhy.dreamingfishcore.gameplay.task_system.command;
...
public class Command_Task {
    ...
    private static int executeStatsTotal(CommandContext<CommandSourceStack> context) {
        int totalCompletions = StoryManager.getTotalTaskCompletions();
        int totalUniquePlayers = StoryManager.getTotalUniquePlayers();
        int stageCount = StoryManager.getStageCount();
    ...
    private static int executeStatsStage(CommandContext<CommandSourceStack> context) {
        ...
        String stats = StoryManager.getStageStatisticsString(stageId);
    ...
    private static int executeStatsTask(CommandContext<CommandSourceStack> context) {
        int taskId = IntegerArgumentType.getInteger(context, "taskId");
        String stats = StoryManager.getTaskStatisticsString(taskId);

        context.getSource().sendSuccess(() -> net.minecraft.network.chat.Component.literal(stats), true);

        // 同时显示完成该任务的玩家列表
        java.util.List<String> players = StoryManager.getTaskFinishedPlayers(taskId);
        if (!players.isEmpty()) {
