...
public class ServerScreenUI_PageRenderer {
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            if (tasks != null) {
                for (StoryTaskData task : tasks) {
                    ...
                    if (task.isCompleted()) globalResolved++;
            ...
            renderStageCard(guiGraphics, listX, cardY, listWidth, stage.getStageName(), stage.getStageDescription(),
                finishedCount, totalInStage, stage.getClientPlayerProgressPercentage(),
                stage.getGlobalProgressPercentage(), isHovered, globalResolved, failedCount);
    ...
    private void renderStageCard(GuiGraphics guiGraphics, int x, int y, int width, String stageName,
                                 ...
                                 float personalProgress, float globalProgress, boolean isHovered,
                                 int globalResolved, int failedCount) {
        int innerMargin = 10;
        int cardHeight = 104;
    ...
    private void renderTaskListForStage(GuiGraphics guiGraphics, int listX, int listY, int listWidth,
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            renderTaskCard(guiGraphics, listX, cardY, listWidth, task.getTaskName(), task.getTaskContent(),
                0xFFFFD700, isFinished, task.isFailed(), true, isHovered, finishedCount);
            lastCardY = cardY + cardHeight;
    ...
    private void renderPlayerTaskList(GuiGraphics guiGraphics, int listX, int listY, int listWidth,
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            // 个人任务不显示完成人数（传0）
            renderTaskCard(guiGraphics, listX, cardY, listWidth, task.getTaskName(), task.getTaskContent(),
                0xFF9B59B6, isFinished, false, false, isHovered, 0);
            lastCardY = cardY + cardHeight;
            cardY += cardHeight + cardSpacing;
        }
    ...
    private void renderTaskCard(GuiGraphics guiGraphics, int x, int y, int width,
                                ...
                                boolean isFinished, boolean isFailed, boolean isServerTask, boolean isHovered,
                                int finishedCount) {
        int innerMargin = 8;
        int cardHeight = 60;
        int stateColor = isFailed ? 0xFFE05B62 : themeColor;

        drawGameCard(guiGraphics, x, y, width, cardHeight, stateColor, isHovered || isFinished || isFailed);
        ...
        drawText(guiGraphics, fullTitle, x + innerMargin + 4, y + innerMargin,
            isFailed ? stateColor : isFinished ? 0xFF888888 : themeColor);
        ...
        if (!isServerTask) {
            ...
            if (isFailed) {
                int markX = countTextX - countWidth - 16;
                drawText(guiGraphics, "X", markX,
                    y + cardHeight - innerMargin - mc.font.lineHeight, stateColor);
            } else if (isFinished) {
