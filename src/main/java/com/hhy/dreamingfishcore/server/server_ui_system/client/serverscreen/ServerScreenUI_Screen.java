...
import com.hhy.dreamingfishcore.server.notice_system.client.Screen_NoticeDetail;
...
public class ServerScreenUI_Screen extends Screen {
    ...
    private void renderStoryStageCard(GuiGraphics guiGraphics, int x, int y, int width, int height,
                                      com.hhy.dreamingfishcore.gameplay.story_system.StoryStageData stage, boolean hovered) {
        ...
        int globalResolved = stage.getCompletedTaskCount();
        int personalResolved = stage.getClientPlayerCompletedTaskCount();
        int progressGap = 8;
        int progressWidth = Math.max(24, (width - 24 - progressGap) / 2);
        int personalX = x + 12 + progressWidth + progressGap;
        drawText(guiGraphics, "全服 " + globalResolved + "/" + total, x + 12, y + 40, TABLET_MUTED_TEXT_COLOR);
        drawText(guiGraphics, "个人 " + personalResolved + "/" + total, personalX, y + 40, TABLET_MUTED_TEXT_COLOR);
    ...
    private void renderStoryStageTasks(GuiGraphics guiGraphics, float mouseX, float mouseY, int x, int y, int width, int height,
        ...
        java.util.List<com.hhy.dreamingfishcore.gameplay.story_system.StoryTaskData> tasks = selectedStage.getTasks();
        ...
        for (int i = 0; i < visible; i++) {
            ...
            renderTaskTerminalCard(guiGraphics, x, currentY, width, TASK_CARD_HEIGHT, task.getTaskName(), task.getTaskContent(),
                task.isClientPlayerFinished(), task.isFailed(), 0xFF7AA8C7, hovered, task.getFinishedPlayerCount());
    ...
    private void renderPersonalTaskCards(GuiGraphics guiGraphics, float mouseX, float mouseY, int x, int y, int width, int height,
                                         java.util.Map<Integer, com.hhy.dreamingfishcore.gameplay.task_system.TaskPlayerData> playerTasks) {
        ...
        int firstY = cardY;
        int lastY = cardY;
        java.util.List<com.hhy.dreamingfishcore.gameplay.task_system.TaskPlayerData> tasks = new java.util.ArrayList<>(playerTasks.values());
        for (int i = 0; i < visible; i++) {
            int taskIndex = i + (int) taskScrollOffset;
            var task = tasks.get(taskIndex);
            int currentY = cardY + i * (TASK_CARD_HEIGHT + gap);
            boolean hovered = mouseX >= x && mouseX <= x + width && mouseY >= currentY && mouseY <= currentY + TASK_CARD_HEIGHT;
            renderTaskTerminalCard(guiGraphics, x, currentY, width, TASK_CARD_HEIGHT, task.getTaskName(), task.getTaskContent(),
                task.isClientPlayerFinished(), false, 0xFF9CB7A8, hovered, 0);
            lastY = currentY + TASK_CARD_HEIGHT;
        }
    ...
    private void renderTaskTerminalCard(GuiGraphics guiGraphics, int x, int y, int width, int height, String title, String content,
                                        boolean finished, boolean failed, int accent, boolean hovered, int finishedPlayers) {
        int stateColor = failed ? 0xFFE05B62 : finished ? 0xFF50D890 : accent;
        ...
        String state = failed ? "FAILED" : finished ? "DONE" : "OPEN";
        ...
        if (finishedPlayers > 0) {
            String doneText = finishedPlayers + "人在场";
                ...
                if (virtualMouseX >= stageArea[0] && virtualMouseX <= stageArea[2] &&
                    ...
                    if (clickedStageIndex >= 0 && clickedStageIndex < sortedStageIds.size()) {
                        ...
                        if (stageIndex < sortedStageIds.size()) {
                            var selectedStage = storyStages.get(sortedStageIds.get(stageIndex));
                            selectedStageId = selectedStage == null ? null : selectedStage.getStageId();
                            taskScrollOffset = 0;
                            return true;
                        }
                    }
                    ...
                    if (clickedCardIndex >= 0 && clickedCardIndex < maxCards) {
                        ...
                        if (taskEntry.isPresent()) {
                            ...
                            if (virtualMouseX >= btnX) {
                                if (!((com.hhy.dreamingfishcore.gameplay.task_system.TaskPlayerData) task).isClientPlayerFinished()) {
                                    // 发送完成任务请求
                                    DreamingFishCore_NetworkManager.sendToServer(
                                        new com.hhy.dreamingfishcore.gameplay.task_system.network.Packet_SyncCompleteTask(taskId, false)
                                    );
                                    return true;
                                }
                            }
